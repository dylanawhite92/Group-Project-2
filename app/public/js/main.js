...
var salary = 0
  ...
  function renderTeamPlayers(num) {
    $.get("/api/league_data", function(data) {
      for (var i = 0; i < data.length; i++) {
        if (
          data[i].team_name == num ||
          data[i].player_position == num ||
          data[i].player_position == `${num}/C` ||
          data[i].player_position == `${num}/F` ||
          data[i].player_position == `${num}/G`
        ) {
          var newDiv = $("<div>");
          newDiv.addClass("active-player");
    ...
    $.get("/api/league_data", function(data) {
      ...
      for (var i = 0; i < data.length; i++) {
        if (num.includes(data[i].player_name)) {
          ...
          $(".player-card").append(
            `Name: ${data[i].player_name} <br>Year: 2018/2019 Salary: $${addCommas(
              currency
            )} 
                    <br>Position: ${data[i].player_position} <br>Team Name: ${
              data[i].team_name
            } 
    ...
    $.get("/api/bulls_data", function(data) {
      for (var i = 0; i < data.length; i++) {
        if (num.includes(data[i].player_name)) {
          ...
          $(".player-card").append(
            `Name: ${data[i].player_name} <br>Year: 2018/2019 <br>Salary: $${addCommas(
              currency
            )} <br>Position: ${data[i].player_position} <br>Team Name: ${
              data[i].team_name
            } <br>Points Per Game: ${data[i].ppg}`
